...
async function GetTransaction(req, res) {
    try {
        ...
        try {
            ...
            // Query transactions for the specific customer
            const query = transactionRef.where('customers', '==', customers);
            const querySnapshot = await query.get();

            // If no transactions found
            if (querySnapshot.empty) {
                return res.status(404).json({ success: false, message: 'No transactions found for the specified customer.' });
            }
